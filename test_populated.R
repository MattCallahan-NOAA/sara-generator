#use web service to pull previous years
library(tidyverse)
library(httr)
hq<-httr::content(
...
#-----------------------------------------------------------------------------#
# Save the below code to the SARA file
#-----------------------------------------------------------------------------#
...
lapply(saralist, write, "saralist.txt", append=TRUE, ncolumns=1000)
...
#chat gpt solution
output_with_comments <- capture.output(saralist)

#save list contents
write(output_with_comments, "saralist2.txt", sep="\n", append=TRUE)

#try saving comments as text
saralist3<-list(
  "#Stock Assessment Results Archive (SARA) file for stocks managed under the North Pacific Fisheries Management Council",
  "#ASSESSMENT_SUMMARY -------------------------------------------------------------------------------------------------",
  "#STOCK - Short code for stock name, part of unique identifier key for joining SARA data, see lookup table for name code",
...
lapply(saralist3, write, "saralist3.txt", append=TRUE, ncolumns=1000)
